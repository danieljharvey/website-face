...
website :: StyleTree
website = Div [Width 100, Height 200] [ Title [Color "red"] "My interesting website"
...
getStyleNumber styles style = (index, allStyles) where
    index = elemIndex style allStyles
    allStyles = if isJust $ elemIndex style styles
              then styles
              else styles <> [style]
...
renderStyleTree = html ++ "/n/n" ++ "<style>" ++ renderAllCSS css ++ "</style>"
    where (html, css) = runState (stateStyleTree website) []
